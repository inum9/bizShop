...
const login = asyncHandler(async (req, res) => {
    ...
    if (!existingUser || !(await existingUser.isPasswordCorrect(password, existingUser.password))) {
        throw new ApiError(401, "Incorrect email or password");
    }

    // FIX 2: Await the token generation function
    // FIX 6: Pass 'res' to the token generation function if it handles cookies
    const { accessToken } = await generateAccessAndRefereshTokens(existingUser._id, res); // We now generate both and set cookies inside it. Only need accessToken here for response body.
